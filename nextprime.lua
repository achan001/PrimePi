...
local function sqrmod(a, n)         -- a^2 (mod n)
    local b = a*a
    if b <= 0x1fffffffffffff then return b%n end
    if a >= 0x16a09e6c000000 then a=n-a end -- ~ 52.5 bits
    b = a % 2^26
    a = a - b
    a = (a*a)%n - n + (2*a*b)%n
    if a <= 0x10000007ffffff then return (a + b*b) % n end
    return (a - n + b*b) % n
end
...
local function mulmod(a, b, n)      -- a*b (mod n)
    local m = a*b
    if m <= 0x1fffffffffffff then return m%n end
    if a >= 0x16a09e6c000000 then a=n-a; b=n-b end
    if b >= 0x16a09e6c000000 then   -- ~ 52.5 bits
        a = mulmod(a, n-b, n)
...
local function nextprime(n)
    ...
    if n > 0x1fffffffffff90 then return 1/0 end
    n = n - (n-1)%2
...
local function prevprime(n)
    ...
    if n > 0x1fffffffffffff then return 1/0 end
    n = n + (n-1)%2
    repeat n=n-2 until isprime(n)
    return n
end
